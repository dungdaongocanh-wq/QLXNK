            ...
            <table class="w-full text-sm">
                <thead class="bg-gray-50 border-b border-gray-100">
                    <tr>
                        ...
                        <th class="px-5 py-3 text-left font-semibold text-gray-600">Công ty xuất</th>
                        <th class="px-5 py-3 text-left font-semibold text-gray-600">Công ty nhập</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-5 py-3 text-gray-600 text-xs">{{ $decl->exporter_name ?? ($decl->customer?->name ?? '—') }}</td>
                        <td class="px-5 py-3 text-gray-600 text-xs">{{ $decl->importer_name ?? '—' }}</td>
                        ...
                        <td class="px-5 py-3">
                            @if($decl->importDeclaration)
                                <a href="{{ route('import-declarations.show', $decl->importDeclaration) }}"
...
<div id="excelModal" class="hidden fixed inset-0 bg-black/50 flex items-center justify-center z-50">
    <div class="bg-white rounded-xl shadow-xl w-full max-w-md p-6">
        <div class="flex items-center justify-between mb-4">
            <h3 class="text-lg font-semibold text-gray-800">Import file Excel tờ khai tạm xuất</h3>
        ...
        <form action="{{ route('export-declarations.upload-excel') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center">
                ...
                <label class="cursor-pointer">
                    ...
                    <input type="file" name="excel_file" id="exportFileInput" accept=".xlsx,.xls" class="hidden" required>
                </label>
                <p id="exportFileName" class="text-xs text-gray-500 mt-1">Hỗ trợ .xlsx, .xls</p>
            </div>
...
@push('scripts')
<script>
    const exportModal = document.getElementById('excelModal');
    document.getElementById('openExcelModal').addEventListener('click', () => exportModal.classList.remove('hidden'));
    document.getElementById('closeExcelModal').addEventListener('click', () => exportModal.classList.add('hidden'));
    document.getElementById('cancelExcelModal').addEventListener('click', () => exportModal.classList.add('hidden'));

    // Hiển thị tên file khi chọn
    document.getElementById('exportFileInput').addEventListener('change', function () {
        const label = document.getElementById('exportFileName');
